from app.responses.user import (
    ProfileResponse, UserResponse, SingleUserResponse, Pagination, PaginatedUserResponse,
...
def test_pagination_and_paginated_user_response():
    pag = Pagination(current_page=1, last_page=2, first_item=1, last_item=10, items_per_page=10, total=20)
    ur = UserResponse(id=1, username="u", email="e")
...
def test_user_create_update_response():
    cr = UserCreateResponse(id=1, username="u", email="e", created_at="now", updated_at="now")
    assert cr.id == 1
    assert cr.username == "u"
    assert cr.email == "e"
    ur = UserUpdateResponse(id=2, username="u2", email="e2", created_at="now", updated_at="now")
    assert ur.id == 2
