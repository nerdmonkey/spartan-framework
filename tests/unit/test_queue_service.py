...
def test_create_queue_with_attributes(queue_service, mock_sqs):
    ...
    mock_sqs.create_queue.assert_called_with(
        QueueName="test-queue", Attributes=custom_attributes
    )
...
def test_send_message_batch_standard_queue(
    queue_service, mock_sqs, sample_queue_url, sample_messages
):
    ...
    response = queue_service.send_message_batch(
        sample_queue_url, sample_messages
    )
...
def test_send_message_batch_fifo_queue(
    queue_service, mock_sqs, sample_queue_url, sample_messages
):
    ...
    queue_service.send_message_batch(fifo_url, sample_messages, "group1")
...
def test_receive_messages_empty_queue(
    queue_service, mock_sqs, sample_queue_url
):
...
def test_delete_message_batch_success(
    queue_service, mock_sqs, sample_queue_url
):
...
def test_purge_queue_error(queue_service, mock_sqs, sample_queue_url):
    ...
    mock_sqs.purge_queue.side_effect = ClientError(
        {"Error": {"Code": "Error", "Message": "Purge failed"}}, "PurgeQueue"
    )
...
def test_get_queue_attributes_success(
    queue_service, mock_sqs, sample_queue_url
):
    ...
    mock_sqs.get_queue_attributes.assert_called_with(
        QueueUrl=sample_queue_url, AttributeNames=["All"]
    )
