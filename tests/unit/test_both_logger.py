...
def both_logger(monkeypatch, mock_file_logger, mock_stream_logger):
    with patch("app.helpers.logger.both.FileLogger", return_value=mock_file_logger), \
         patch("app.helpers.logger.both.StreamLogger", return_value=mock_stream_logger):
...
def test_log_calls_both_loggers(both_logger, mock_file_logger, mock_stream_logger):
...
def test_info_calls_both_loggers(both_logger, mock_file_logger, mock_stream_logger):
    both_logger.info("info message", extra={"a": 1})
    mock_file_logger.info.assert_called_once_with("info message", extra={"a": 1})
...
def test_warning_calls_both_loggers(both_logger, mock_file_logger, mock_stream_logger):
    both_logger.warning("warn message", extra={"b": 2})
    mock_file_logger.warning.assert_called_once_with("warn message", extra={"b": 2})
    args, kwargs = mock_stream_logger.warning.call_args
    assert "warn message" in args[0]
    assert "extra" in args[0]

def test_error_calls_both_loggers(both_logger, mock_file_logger, mock_stream_logger):
    both_logger.error("error message", extra={"c": 3})
    mock_file_logger.error.assert_called_once_with("error message", extra={"c": 3})
    args, kwargs = mock_stream_logger.error.call_args
    assert "error message" in args[0]
    assert "extra" in args[0]

def test_debug_calls_both_loggers(both_logger, mock_file_logger, mock_stream_logger):
    both_logger.debug("debug message", extra={"d": 4})
    mock_file_logger.debug.assert_called_once_with("debug message", extra={"d": 4})
    args, kwargs = mock_stream_logger.debug.call_args
    assert "debug message" in args[0]
    assert "extra" in args[0]

def test_exception_calls_both_loggers(both_logger, mock_file_logger, mock_stream_logger):
    both_logger.exception("exception message", extra={"e": 5})
    mock_file_logger.exception.assert_called_once_with("exception message", extra={"e": 5})
    mock_stream_logger.exception.assert_called_once_with("exception message", extra={"e": 5})

def test_logger_init_sets_attributes(monkeypatch):
    with patch("app.helpers.logger.both.FileLogger") as file_patch, \
        ...
        stream_patch.assert_called_once_with(service_name="svc", level="WARNING")
