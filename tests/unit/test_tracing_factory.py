...
def test_tracer_factory_selects_gcloud_when_gcp_env(monkeypatch):
    ...
    monkeypatch.setattr("app.helpers.environment.env", lambda k=None, d=None: {
    ...
    monkeypatch.setattr(mod_gcloud, "trace_v1", SimpleNamespace(TraceServiceClient=lambda: SimpleNamespace()), raising=False)
...
def test_tracer_factory_returns_local_when_local_env(monkeypatch):
    monkeypatch.setattr("app.helpers.environment.env", lambda k=None, d=None: {"APP_ENVIRONMENT": "local", "APP_NAME": "svc"}.get(k, d))
