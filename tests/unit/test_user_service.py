...
def test_save_user(user_service, user_create_request, mock_db_session):
    mock_db_session.query.return_value.filter.return_value.first.return_value = (
        None  # Ensure user does not exist
    )
    mock_user = MagicMock()
    mock_user.id = 1
    ...
    mock_db_session.add.side_effect = lambda x: setattr(
        x, "created_at", "2023-01-01 00:00:00"
    )
    mock_db_session.add.side_effect = lambda x: setattr(
        x, "updated_at", "2023-01-01 00:00:00"
    )
...
def test_save_duplicate_user(
    user_service, user_create_request, mock_db_session
):
    mock_user = MagicMock()
    mock_user.username = "testuser"
    mock_user.email = "testuser@example.com"
...
def test_update_user_not_found(
    user_service, user_update_request, mock_db_session
):
    mock_db_session.query.return_value.filter.return_value.first.return_value = (
        None
    )
