...
class FakeUser:
    ...
    def __init__(self, username, email, password, created_at=None, updated_at=None, id=None):
        self.id = id
...
from app.exceptions.user import DuplicateUserError, UserNotFoundException, InvalidSortFieldException
...
def test_save_and_get_by_id(user_service):
    resp = user_service.save(UserCreateRequest(username="bob", email="bob@example.com", password="pw"))
    user = user_service.get_by_id(resp.id)
    assert user.username == "bob"
    assert user.email == "bob@example.com"

def test_save_duplicate_email(user_service):
    user_service.save(UserCreateRequest(username="bob", email="bob@example.com", password="pw"))
    with pytest.raises(DuplicateUserError):
        user_service.save(UserCreateRequest(username="bob2", email="bob@example.com", password="pw"))
...
def test_bulk_delete(user_service):
    ...
    ids = []
    for i in range(3):
        resp = user_service.save(UserCreateRequest(username=f"user_bulk_{i}", email=f"bulk_{i}@x.com", password="pw"))
        ids.append(resp.id)
...
def test_all_and_total(user_service):
    ...
    user_service.db.last_id = 0
    for i in range(5):
        user_service.save(UserCreateRequest(username=f"user_total_{i}", email=f"total_{i}@x.com", password="pw"))
    users, total, last_page, first_item, last_item = user_service.all(page=1, items_per_page=2)
    assert len(users) == 2
    assert total == 5
    assert last_page == 3
    assert first_item == 1
