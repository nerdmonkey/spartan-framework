...
def test_gcp_happy_path_redaction_and_structured_logging(monkeypatch):
    """GCPLogger should create structured logs and redact sensitive fields."""
    import sys

    # Patch sys.modules so imports in gcp.py resolve to stubs
    ...
    mod = __import__("app.services.logging.gcp", fromlist=["*"])
    ...
    gw = GCPLogger(service_name="svc", level="INFO", sample_rate=1.0)
...
def test_gcp_sampling_blocks_logging(monkeypatch):
    ...
    gw = GCPLogger(service_name="svc", level="INFO")
...
def test_gcp_exception_logging_and_fallback(monkeypatch):
    ...
    gw = GCPLogger(service_name="svc", level="ERROR")
